...
/* PATCH /api/issues/:id/complete — assigned faculty marks the project solved, final immediately */
const markComplete = asyncHandler(async (req, res) => {
  const issue = await Issue.findById(req.params.id);
  if (!issue) return res.status(404).json({ message: "Issue not found" });
  if (!["team_formed", "seeking_funds", "funded", "in_progress"].includes(issue.status)) {
    return res.status(400).json({ message: "This issue can't be marked complete from its current status" });
  }
  const faculty = await FacultyProfile.findOne({ user: req.user._id });
  if (!faculty || String(issue.assignedFaculty) !== String(faculty._id)) {
    return res.status(403).json({ message: "Only the assigned faculty can mark this complete" });
  }
  await moveStatus(issue, "solved", req.user._id, "Marked complete by faculty");
  res.json({ issue });
});
